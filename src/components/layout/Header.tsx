        ...
        <div className="ml-auto flex items-center space-x-4">
              ...
              <div className="absolute right-0 mt-2 w-80 origin-top-right rounded-lg bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none animate-in fade-in zoom-in-95 duration-100 overflow-hidden z-50">
                    ...
                    notifications.map((item) => (
                      ...
                      >
                        <div className="flex gap-3">
                          <div
                            className={`mt-1 p-2 rounded-full h-fit ${user?.role === "KDUS" ? "bg-yellow-100 text-yellow-600" : "bg-green-100 text-green-600"}`}
                          >
                            {user?.role === "KDUS" ? (
                              <AlertCircle size={16} />
                            ) : (
                ...
                {notifications.length > 0 && (
                  <Link
                    to="/reports"
                    onClick={() => setIsNotifOpen(false)} 
                    className="block text-center text-xs text-green-600 hover:text-green-800 py-2 border-t border-gray-100 font-medium"
                  >
                    Lihat Semua Laporan
                  </Link>
                )}
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              className="flex items-center space-x-3 rounded-full border border-transparent p-1 transition-all hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-100"
            >
              <div className="h-9 w-9 rounded-full bg-green-600 flex items-center justify-center text-white font-semibold shadow-sm">
                {avatarChar}
              </div>
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-lg bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none animate-in fade-in zoom-in-95 duration-100 z-50">
                ...
                <div className="p-1">
                  <Link
                    to="/profile"
                    className="flex w-full items-center px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-600 rounded-md transition-colors"
